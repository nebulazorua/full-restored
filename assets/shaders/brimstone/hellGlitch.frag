...
GlitchSeed glitchSeed(vec2 p, float speed, float proba) {
    float seedTime = floor(time * speed);
    vec2 seed = vec2(
        1. + mod(seedTime / 100., 100.),
        1. + mod(seedTime, 100.)
...
void glitchSwap(inout vec2 p) {
    ...
    apply = shouldApply(seed);
    swapBlocks(p, groupSize, subGrid, blockSize, seed.seed, apply*0.5);
}
...
vec4 transverseChromatic(vec2 p) {
    ...
    vec4 newColor = vec4(accumulator / float(sampleCount), flixel_texture2D(bitmap, destCoord).a);
	return newColor;
}
